...
public partial class Assessment : ContentPage
{
    ...
    public Assessment(Courses course)
    {
        InitializeComponent();

        DisplayAlert("Alert", "In order to edit, please double-click the designated assessment..", "OK");
    ...
    private async void RadioButton_CheckedChanged_1(object sender, CheckedChangedEventArgs e)
    {
        if (e.Value && selectedCourse != null)
        {
            var course = (Courses)((Button)sender).BindingContext;
            await Navigation.PushAsync(new PerformanceAssessment(course));
        }
    }
